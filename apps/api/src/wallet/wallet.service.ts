...
interface ItemRow extends RowDataPacket { id: string; sku: string; name: string; description: string; category: string; price_coins: number; price_pips: number; asset_key: string; is_giftable: number; is_active: number; stock: number | null; }
...
export class WalletService {
  ...
  async catalog() {
    ...
    return rows.map((row) => ({ ...row, priceCoins: Number(row.priceCoins), pricePips: Number(row.pricePips), stock: row.stock === null ? null : Number(row.stock) }));
  }

  async inventory(userId: string) {
    const rows = await this.mysql.query<RowDataPacket[]>(`SELECT i.id, i.shop_item_id AS itemId, i.quantity, i.equipped, i.acquired_at AS acquiredAt, i.expires_at AS expiresAt, s.sku, s.name, s.description, s.category, s.asset_key AS assetKey FROM inventory_items i JOIN shop_items s ON s.id = i.shop_item_id WHERE i.user_id = ? AND (i.expires_at IS NULL OR i.expires_at > UTC_TIMESTAMP(3)) ORDER BY i.acquired_at DESC`, [userId]);
  ...
  async buy(userId: string, dto: BuyItemDto) {
    return this.mysql.transaction(async (connection) => {
      if (dto.idempotencyKey) {
        ...
        if (already[0]) return { success: true, replayed: true, transactionId: already[0].id, balance: await this.balanceOnConnection(connection, userId) };
      }
      ...
      if (item.stock !== null && item.stock < 1) throw conflict('This item is sold out.');
      ...
      if (item.stock !== null) await connection.execute(`UPDATE shop_items SET stock = stock - 1 WHERE id = ? AND stock > 0`, [dto.itemId]);
      ...
      return { success: true, replayed: false, transactionId: txId, item: { id: item.id, name: item.name }, balance: await this.balanceOnConnection(connection, userId) };
    });
  }

  async equip(userId: string, dto: EquipItemDto) {
    const rows = await this.mysql.query<RowDataPacket[]>(`SELECT s.category FROM inventory_items i JOIN shop_items s ON s.id = i.shop_item_id WHERE i.user_id = ? AND i.shop_item_id = ? AND i.quantity > 0`, [userId, dto.itemId]);
    if (!rows[0]) throw notFound('You do not own this item.');
    await this.mysql.transaction(async (connection) => {
      await connection.execute(`UPDATE inventory_items i JOIN shop_items s ON s.id = i.shop_item_id SET i.equipped = FALSE WHERE i.user_id = ? AND s.category = ?`, [userId, rows[0].category]);
  ...
  async gift(senderId: string, dto: GiftItemDto) {
    ...
    return this.mysql.transaction(async (connection) => {
      const recipient = await connection.query<RowDataPacket[]>(`SELECT id FROM users WHERE id = ? AND status = 'active'`, [dto.recipientId]);
      if (!recipient[0]) throw notFound('Recipient not found.');
      const item = await this.lockItem(connection, dto.itemId);
      ...
      if (owned < dto.quantity) throw forbidden('You do not own enough of this item.');
      await connection.execute(`UPDATE inventory_items SET quantity = quantity - ? WHERE user_id = ? AND shop_item_id = ?`, [dto.quantity, senderId, dto.itemId]);
      await connection.execute(`INSERT INTO inventory_items (id, user_id, shop_item_id, quantity) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity)`, [randomUUID(), dto.recipientId, dto.itemId, dto.quantity]);
      const giftId = randomUUID();
      await connection.execute(`INSERT INTO gift_transactions (id, sender_id, recipient_id, shop_item_id, quantity, note) VALUES (?, ?, ?, ?, ?, ?)`, [giftId, senderId, dto.recipientId, dto.itemId, dto.quantity, dto.note?.trim() ?? null]);
      return { success: true, giftId, item: item.name, quantity: dto.quantity };
    });
  }
