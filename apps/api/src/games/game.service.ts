...
export class GameService implements OnModuleInit {
  ...
  async replay(matchId: string, viewerId: string) {
    ...
    return rows.map((row) => {
      const stateAfter = typeof row.stateAfter === 'string' ? JSON.parse(row.stateAfter) as GameState : JSON.parse(JSON.stringify(row.stateAfter)) as GameState;
      // Server-side physics seeds are replay inputs, not client-visible game
      // state. Keeping them out of replays prevents a viewer from predicting
      // future pocket/foul resolutions.
      delete stateAfter.serverSeed;
      return { ...row, payload: typeof row.payload === 'string' ? JSON.parse(row.payload) : row.payload, stateAfter };
    });
  }
