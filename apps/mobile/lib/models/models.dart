...
class MatchModel {
  const MatchModel({required this.id, required this.gameId, required this.status, required this.revision, required this.state, required this.players, required this.winnerIds, required this.draw, this.conversationId});
  final String id; final String gameId; final String status; final int revision; final Map<String, dynamic> state; final List<Map<String, dynamic>> players; final List<String> winnerIds; final bool draw; final String? conversationId;
  factory MatchModel.fromJson(Map<String, dynamic> json) => MatchModel(id: json['id'] as String, gameId: json['gameId'] as String, status: json['status'] as String? ?? 'active', revision: (json['revision'] as num?)?.toInt() ?? 0, state: Map<String, dynamic>.from(json['state'] as Map? ?? const {}), players: (json['players'] as List? ?? const []).map((item) => Map<String, dynamic>.from(item as Map)).toList(), winnerIds: (json['winnerIds'] as List? ?? const []).map((item) => item.toString()).toList(), draw: json['draw'] is bool ? json['draw'] as bool : json['draw'] == 1, conversationId: json['conversationId'] as String?);
}
