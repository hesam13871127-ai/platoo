...
class _ShopStatePanel extends StatelessWidget {
  const _ShopStatePanel({required this.icon, required this.title, required this.message});
  final IconData icon;
  final String title;
  final String message;
  @override
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(30), child: Column(mainAxisSize: MainAxisSize.min, children: [Container(width: 62, height: 62, alignment: Alignment.center, decoration: BoxDecoration(color: AppTheme.violet.withOpacity(.12), shape: BoxShape.circle), child: Icon(icon, color: AppTheme.violet, size: 28)), const SizedBox(height: 13), Text(title, style: Theme.of(context).textTheme.titleMedium, textAlign: TextAlign.center), const SizedBox(height: 6), Text(message, style: Theme.of(context).textTheme.bodySmall, textAlign: TextAlign.center)])));
}
    ...
    return SafeArea(
      child: Padding(
        ...
        child: inventory.when(
          ...
          error: (error, _) => _ShopStatePanel(icon: Icons.cloud_off_rounded, title: 'Inventory unavailable', message: 'Try again in a moment.'),
          data: (items) => items.isEmpty
              ? const _ShopStatePanel(icon: Icons.backpack_outlined, title: 'Your inventory is empty', message: 'Buy a cosmetic and it will appear here.')
              : ListView.separated(
                  shrinkWrap: true,
