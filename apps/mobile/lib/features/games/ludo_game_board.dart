    ...
    return Card(
      child: Padding(
        ...
        child: Column(children: [
          Row(children: [
            const Icon(Icons.casino_rounded, color: AppTheme.coral),
            const SizedBox(width: 8),
            const Text('Ludo', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
            const Spacer(),
            if (roll != null) _Die(value: roll),
          ]),
          const SizedBox(height: 7),
          Align(alignment: Alignment.centerLeft, child: Text(_status(isTurn, roll, legal), style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontWeight: FontWeight.w700))),
          ...
          AspectRatio(
            ...
            child: LayoutBuilder(builder: (context, constraints) {
              ...
              return Stack(children: [
                Positioned.fill(child: CustomPaint(painter: _LudoPainter(track: track, safeSquares: safeSquares, colors: colors, finishLanes: finishLanes))),
          ...
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: isTurn && roll == null ? () => onAction({'type': 'roll'}) : null, icon: const Icon(Icons.casino_rounded), label: Text(roll == null ? 'Roll dice' : 'Choose a token'))),
            if (roll != null && legal.isEmpty) ...[
              const SizedBox(width: 9),
              OutlinedButton(onPressed: isTurn ? () => onAction({'type': 'pass'}) : null, child: Text(roll == 6 ? 'Roll again' : 'Pass')),
            ],
          ]),
          const SizedBox(height: 10),
          Wrap(spacing: 12, runSpacing: 6, children: [for (var side = 0; side < positions.length && side < 4; side += 1) _PlayerKey(color: colors[side], label: _playerName(side))]),
        ]),
      ),
    );
  }
    ...
    return Positioned(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(color: colors[side].withOpacity(.96), shape: BoxShape.circle, border: Border.all(color: selectable ? Colors.white : Colors.black26, width: selectable ? 3 : 1), boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 3, offset: Offset(1, 2))]),
          child: Center(child: Text('${token + 1}', style: TextStyle(fontSize: cell * .25, color: Colors.white, fontWeight: FontWeight.w900))),
        ),
      ),
    );
  }
...
class _Die extends StatelessWidget {
  ...
  final int value;
  @override
  Widget build(BuildContext context) => Container(width: 34, height: 34, alignment: Alignment.center, decoration: BoxDecoration(color: AppTheme.coral.withOpacity(.13), borderRadius: BorderRadius.circular(10)), child: Text('$value', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 17)));
}

class _PlayerKey extends StatelessWidget {
  const _PlayerKey({required this.color, required this.label});
  final Color color;
  final String label;
  @override
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 10, height: 10, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 5), Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700))]);
}

class _LudoPainter extends CustomPainter {
  const _LudoPainter({required this.track, required this.safeSquares, required this.colors, required this.finishLanes});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(board, Paint()..color = const Color(0xFFF8F4ED));
    for (var side = 0; side < 4; side += 1) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(12)), Paint()..color = colors[side].withOpacity(.18));
      final inner = rect.deflate(cell * .8);
      canvas.drawRRect(RRect.fromRectAndRadius(inner, const Radius.circular(10)), Paint()..color = Colors.white.withOpacity(.85));
    }
    for (var index = 0; index < track.length; index += 1) {
      ...
      final paint = Paint()..color = safeSquares.contains(index) ? const Color(0xFFFFE7BD) : Colors.white;
      canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(4)), paint);
      if (safeSquares.contains(index)) _drawStar(canvas, rect.center, cell * .18, const Color(0xFFB77A41));
    }
    for (var side = 0; side < 4; side += 1) {
      for (var index = 0; index < finishLanes[side].length; index += 1) {
        ...
        canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(4)), Paint()..color = colors[side].withOpacity(.25));
      }
    }
  ...
  @override
  bool shouldRepaint(covariant _LudoPainter oldDelegate) => oldDelegate.track != track || oldDelegate.safeSquares != safeSquares;
}
