      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Expanded(child: VibeText('Dice Party', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 19))),
            _DiceBadge(label: finished ? 'Final' : 'Round $round / $maxRounds'),
          ]),
          const SizedBox(height: 6),
          VibeText(finished ? 'The dice have settled.' : 'Highest total after $maxRounds rounds wins.', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12, fontWeight: FontWeight.w700)),
          if (lastRoll != null && !finished) ...[
            const SizedBox(height: 10),
            Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9), decoration: BoxDecoration(color: AppTheme.gold.withOpacity(.1), borderRadius: BorderRadius.circular(12)), child: VibeText('${_name(lastRoll['playerId']?.toString())} rolled a ${lastRoll['value'] ?? '–'}', style: const TextStyle(color: AppTheme.gold, fontWeight: FontWeight.w800, fontSize: 12))),
          ],
          ...
          SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: isTurn ? () => onAction({'type': 'roll'}) : null, icon: const Icon(Icons.casino_rounded), label: VibeText(isTurn ? 'Roll the dice' : finished ? 'Match complete' : 'Waiting for turn'))),
          const SizedBox(height: 14),
          const VibeText('Standings', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
          const SizedBox(height: 8),
          ..._standings(scores, leader, finished),
          if (history.isNotEmpty && !finished) ...[
            const SizedBox(height: 12),
            const VibeText('Recent rolls', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
            const SizedBox(height: 6),
            for (final entry in history)
              if (entry is Map)
                Padding(padding: const EdgeInsets.only(bottom: 3), child: VibeText('Round ${entry['round'] ?? '–'} · ${_name(entry['playerId']?.toString())} rolled ${entry['value'] ?? '–'}', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 11, fontWeight: FontWeight.w700))),
          ],
        ]),
      ),
    );
        ...
        Builder(builder: (context) {
          ...
          return Padding(
            ...
            child: Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 9), decoration: BoxDecoration(color: highlight ? AppTheme.gold.withOpacity(.13) : Theme.of(context).colorScheme.surface, borderRadius: BorderRadius.circular(12), border: Border.all(color: highlight ? AppTheme.gold.withOpacity(.5) : Theme.of(context).dividerColor)), child: Row(children: [
              VibeText('${rank + 1}', style: TextStyle(fontWeight: FontWeight.w900, color: highlight ? AppTheme.gold : Theme.of(context).colorScheme.onSurfaceVariant)),
              const SizedBox(width: 10),
              Expanded(child: VibeText(name, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13))),
              if (won) const Padding(padding: EdgeInsets.only(right: 6), child: Icon(Icons.emoji_events_rounded, size: 16, color: AppTheme.gold)),
              VibeText('$score', style: const TextStyle(fontWeight: FontWeight.w900)),
            ])),
          );
        }),
    ];
...
class _PlayerDie extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(children: [
          ...
          VibeText(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 11)),
        ]),
      );
}
...
class _DieFace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: value == null
          ? Center(child: VibeText('?', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Theme.of(context).disabledColor)))
