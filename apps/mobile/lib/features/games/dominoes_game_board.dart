    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _ActionButton(label: 'Draw', icon: Icons.add_rounded, enabled: canDraw, onPressed: () { setState(() => selectedIndex = null); widget.onAction({'type': 'draw'}); })),
            const SizedBox(width: 8),
            Expanded(child: _ActionButton(label: 'Pass', icon: Icons.skip_next_rounded, enabled: canPass, onPressed: () { setState(() => selectedIndex = null); widget.onAction({'type': 'pass'}); })),
          ]),
          ...
          if (selected != null && isTurn && !finished) ...[
            const SizedBox(height: 12),
            _PlacementControls(left: chain.isNotEmpty && selected.contains(chain.first), right: chain.isNotEmpty && selected.contains(chain.last), onPlace: (side) { final index = selectedIndex; if (index == null) return; setState(() => selectedIndex = null); widget.onAction({'type': 'play', 'index': index, 'side': side}); }),
          ],
