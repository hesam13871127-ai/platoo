      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Expanded(child: VibeText('Bingo', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 19))),
            if (lastNumber != null) _NumberBadge(number: lastNumber),
          ]),
          const SizedBox(height: 12),
          VibeText(finished ? 'The cage is closed. Check the result above.' : isTurn ? 'Your turn · draw the next number.' : 'Numbers are marked automatically as they are called.', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12, fontWeight: FontWeight.w700)),
          const SizedBox(height: 13),
          _BingoCard(values: values, marked: marked),
          const SizedBox(height: 13),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: isTurn && !finished ? () => onAction({'type': 'draw'}) : null, icon: const Icon(Icons.casino_rounded), label: VibeText(isTurn ? 'Draw number' : 'Waiting for turn'))),
            const SizedBox(width: 10),
            VibeText('${called.length} / 75', style: TextStyle(fontWeight: FontWeight.w900, color: Theme.of(context).colorScheme.onSurfaceVariant)),
          ]),
          if (called.isNotEmpty) ...[
            const SizedBox(height: 14),
            VibeText('Called numbers', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Theme.of(context).colorScheme.onSurfaceVariant)),
    ...
    return DecoratedBox(
      ...
      child: Padding(
        ...
        child: Column(children: [
          Row(children: headings.map((heading) => Expanded(child: Center(child: VibeText(heading, style: const TextStyle(color: AppTheme.coral, fontWeight: FontWeight.w900))))).toList()),
            ...
            child: GridView.builder(
              ...
              itemBuilder: (_, index) {
                ...
                return AnimatedContainer(
                  ...
                  child: Center(child: value == 0 ? const Icon(Icons.auto_awesome_rounded, size: 17, color: Colors.white) : VibeText('$value', style: TextStyle(fontWeight: FontWeight.w900, color: isMarked ? Colors.white : const Color(0xFF332A35), fontSize: 15))),
                );
              },
            ),
          ),
...
class _NumberBadge extends StatelessWidget {
  ...
  final int number;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 8), decoration: BoxDecoration(color: AppTheme.coral, borderRadius: BorderRadius.circular(13)), child: VibeText('$number', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 17)));
}
