...
class _GameCanvasState extends State<GameCanvas> { int? selected; final word = TextEditingController(); @override void dispose() { word.dispose(); super.dispose(); }
  ...
  Widget _ocho(BuildContext context) { final hands = widget.state['hands'] as List? ?? const []; final hand = hands.isNotEmpty ? hands[0] as List : const []; return Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Text('Your hand', style: TextStyle(fontWeight: FontWeight.w900)), const Spacer(), Text('Top: ${_topCardValue()}')]), const SizedBox(height: 14), Wrap(spacing: 8, runSpacing: 8, children: [for (var i = 0; i < hand.length; i++) _CardButton(card: Map<String, dynamic>.from(hand[i] as Map), onTap: () => widget.onAction({'type': 'play', 'index': i, if ((hand[i] as Map)['color'] == 'wild') 'color': 'red'}))]), const SizedBox(height: 14), OutlinedButton.icon(onPressed: () => widget.onAction({'type': 'draw'}), icon: const Icon(Icons.add_rounded), label: const Text('Draw card'))]))); }
  String _topCardValue() { final discard = widget.state['discard'] as List? ?? const []; if (discard.isEmpty) return 'card'; final top = discard.last; return top is Map ? top['value']?.toString() ?? 'card' : 'card'; }
...
class _CardButton extends StatelessWidget { const _CardButton({required this.card, required this.onTap}); final Map<String, dynamic> card; final VoidCallback onTap; @override Widget build(BuildContext context) { final color = card['color'] == 'red' ? AppTheme.coral : card['color'] == 'yellow' ? AppTheme.gold : card['color'] == 'green' ? AppTheme.mint : AppTheme.violet; return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(12), child: Container(width: 52, height: 72, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(12), boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 4, offset: Offset(0, 3))]), child: Center(child: Text('${card['value']}', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900))))) ; } }
