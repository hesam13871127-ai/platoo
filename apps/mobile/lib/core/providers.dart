...
class SettingsController extends Notifier<({AppLocale locale, ThemeChoice theme})> {
  @override
  ({AppLocale locale, ThemeChoice theme}) build() => (locale: AppLocale.en, theme: ThemeChoice.system);
...
class AuthController extends AsyncNotifier<AuthSession?> {
  ...
  Future<AuthSession?> build() async {
    ...
    try {
      ...
      final session = AuthSession(accessToken: token, refreshToken: await ref.read(tokenStoreProvider).refreshToken() ?? '', expiresAt: DateTime.now().add(const Duration(minutes: 15)), user: user);
      _syncPreferences(user);
      return session;
    } catch (error) {
  ...
  Future<void> verifyOtp(String challengeId, String code) async {
    ...
    state = await AsyncValue.guard(() async {
      final data = await _api.post('/auth/otp/verify', data: {'challengeId': challengeId, 'code': code}) as Map;
  ...
  Future<void> social(String provider, String token, {String? displayName}) async {
    state = const AsyncLoading();
    state = await AsyncValue.guard(() async {
      final data = await _api.post('/auth/social', data: {'provider': provider, 'token': token, if (displayName != null) 'displayName': displayName}) as Map;
      final session = AuthSession.fromJson(Map<String, dynamic>.from(data));
      _syncPreferences(session.user);
      await ref.read(tokenStoreProvider).saveSession(session);
      return session;
  ...
  Future<void> updatePreferences({AppLocale? locale, ThemeChoice? theme}) async {
    final payload = <String, dynamic>{if (locale != null) 'locale': locale == AppLocale.fa ? 'fa' : 'en', if (theme != null) 'theme': theme.name};
    final data = await _api.put('/auth/preferences', data: payload) as Map;
    final current = state.value;
    if (current != null) {
      final user = UserProfile.fromJson(Map<String, dynamic>.from(data));
      _syncPreferences(user);
      final next = AuthSession(accessToken: current.accessToken, refreshToken: current.refreshToken, expiresAt: current.expiresAt, user: user);
      state = AsyncData(next);
      await ref.read(tokenStoreProvider).saveSession(next);
    }
  }

  void _syncPreferences(UserProfile user) {
    ref.read(settingsProvider.notifier).setLocale(user.locale);
    ref.read(settingsProvider.notifier).setTheme(user.theme);
  }
}
